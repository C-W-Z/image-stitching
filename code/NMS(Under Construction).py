...
def NMS_Harris(image, Point_limit:int, MinDis:float, k=0.04, Harris_thresRatio=0.01):
    #Point_limit(N): the function will stop after finding N points using NMS
    #MinDis: Used for determine the Threshold for NMS that knows which feature points to delete
    #Harris_thresRatio=0.01: A variable used for Harris-part's unwanted points' deletion 
    ...
    threshold = Harris_thresRatio*np.max(R)
    R[R<threshold] = 0
